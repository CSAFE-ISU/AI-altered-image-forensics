...
def sklearn_mocks(monkeypatch):
    ...
    mod_ensemble = MagicMock()
    mod_ensemble.RandomForestClassifier = MockRF
    mod_model_selection = MagicMock()
    mod_model_selection.StratifiedKFold = MockCV
...
class TestRandomForestRoute:
    ...
    def test_valid_request_returns_200_with_expected_keys(
        self, sklearn_mocks, client, tmp_base
    ):
        (tmp_base / "records.json").write_text(
            json.dumps(_make_dataset()), encoding="utf-8"
        )
    ...
    def test_record_counts_correct(self, sklearn_mocks, client, tmp_base):
        (tmp_base / "records.json").write_text(
            json.dumps(_make_dataset(n_orig=6, n_alt=4)), encoding="utf-8"
        )
    ...
    def test_selected_models_filters_altered_records(
        self, sklearn_mocks, client, tmp_base
    ):
        records = (
    ...
    def test_feature_set_indicators(self, sklearn_mocks, client, tmp_base):
        ...
        (tmp_base / "records.json").write_text(json.dumps(records), encoding="utf-8")
        resp = client.post(
            "/api/random_forest", json={"seed": 1, "feature_set": "indicators"}
        )
        assert resp.status_code == 200
        body = resp.get_json()
    ...
    def test_records_missing_indicators_skipped_for_indicators_feature_set(
        self, sklearn_mocks, client, tmp_base
    ):
    ...
    def test_feature_set_both(self, sklearn_mocks, client, tmp_base):
        ...
        (tmp_base / "records.json").write_text(json.dumps(records), encoding="utf-8")
        resp = client.post(
            "/api/random_forest", json={"seed": 1, "feature_set": "both"}
        )
        assert resp.status_code == 200
        body = resp.get_json()
        feature_names = [f["feature"] for f in body["feature_importances"]]
    ...
    def test_stratify_by_model(self, sklearn_mocks, client, tmp_base):
        ...
        (tmp_base / "records.json").write_text(json.dumps(records), encoding="utf-8")
        resp = client.post(
            "/api/random_forest", json={"seed": 1, "stratify_by": "model"}
        )
        assert resp.status_code == 200
        body = resp.get_json()
    ...
    def test_fold_accuracies_length_matches_n_splits(
        self, sklearn_mocks, client, tmp_base
    ):
        (tmp_base / "records.json").write_text(
            json.dumps(_make_dataset()), encoding="utf-8"
