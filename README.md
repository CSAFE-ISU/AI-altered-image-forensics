...
- **LLM detectors** — paste results from four chat LLMs manually, each in its own
  card: [Claude](https://claude.ai), [Gemini](https://gemini.google.com/app),
  [Grok](https://grok.com/) and [ChatGPT](https://chatgpt.com/). For each one,
  record the model / version, then work through the same fixed list of three
  ...
  four detectors are independent — filling in Gemini never affects Claude.
